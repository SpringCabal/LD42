...
local HEALTH_REGEN_RATE     = 1.0
local FREEZING_HEALTH_DECAY = 1.0
local STARVING_HEALTH_DECAY = 0.2
...
local MAX_FOOD          = 100
local START_FOOD        = 40
local FOOD_DECAY_RATE   = 0.1
local FOOD_EATING_RATE  = 10
...
local MAX_HEAT          = 100
local START_HEAT        = 80
local WARMING_INCREASE  = 1
local COLD_DECREASE     = 0.1
...
function gadget:UnitCreated(unitID)
    if UnitIsEskimo(unitID) then
        local function _l(attr)
            Spring.GetUnitRulesParam(unitID, attr)
        end
...
local function DoFood(eskimo)
    ...
    local food = eskimo.attrs.eating_state
    if eating_state == EATING_STATES.IDLE then
        food = food - EATING_STATES.FOOD_DECAY_RATE
    elseif eating_state == EATING_STATES.EATING then
        food = food + EATING_STATES.FOOD_EATING_RATE
    end

    if food <= 0 then
        SetAttribute(unitID, "eating_state", WARM_STATES.STARVING)
    end
